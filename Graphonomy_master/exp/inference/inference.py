...
def graphonomy_main(loadmodel='', img_path='', output_path='', output_name='', use_gpu=0):
    """
    Main function to run cloth mask inference using Deeplab Xception model.
    
    Args:
        loadmodel (str): Path to the pre-trained model file (default: '')
        img_path (str): Path to the input image (default: '')
        output_path (str): Path to save the output mask (default: '')
        output_name (str): Name of the output mask file (default: '')
        use_gpu (int): Flag to use GPU (1 for True, 0 for False) (default: 0)
    """
    ...
    if not loadmodel == '':
        if torch.cuda.is_available():
            ...
            x = torch.load(loadmodel, map_location=torch.device('cpu'))#cuda
    ...
    inference(net=net, img_path=img_path,output_path=output_path , output_name=output_name, use_gpu=use_gpu)
...
if __name__ == '__main__':
    ...
    if not opts.loadmodel == '':
        if torch.cuda.is_available():
            ...
            x = torch.load(opts.loadmodel, map_location=torch.device('cpu'))#cuda
        net.load_source_model(x)
