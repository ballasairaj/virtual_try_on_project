...
def get_opt(test_name, tocg_checkpoint, gpu_ids, gen_checkpoint, datasetting, data_list, dataroot):
    parser = argparse.ArgumentParser()
    parser.add_argument("--test_name", default=test_name, type=str, help="Name of the test")
    parser.add_argument("--tocg_checkpoint", default=tocg_checkpoint, type=str, help="Path to tocg checkpoint")
    parser.add_argument("--gpu_ids", default=gpu_ids, type=str, help="GPU IDs to use (e.g., '0' or '0,1')")
    parser.add_argument("--gen_checkpoint", default=gen_checkpoint, type=str, help="Path to generator checkpoint")
    parser.add_argument("--datasetting", default=datasetting, type=str, help="Dataset setting (e.g., 'unpaired')")
    parser.add_argument("--data_list", default=data_list, type=str, help="Path to data list file")
    parser.add_argument("--dataroot", default=dataroot, type=str, help="Root directory for dataset")
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            if opt.cuda :
                ...
                # pre_clothes_mask = torch.FloatTensor((pre_clothes_mask.detach().cuda().numpy() > 0.5).astype(np.float)).cuda()
...
def main(test_name, tocg_checkpoint, gpu_ids, gen_checkpoint, datasetting, data_list, dataroot):
    # opt = get_opt()
    opt = get_opt(test_name, tocg_checkpoint, gpu_ids, gen_checkpoint, datasetting, data_list, dataroot)
    #print(opt)
...
if __name__ == "__main__":
    main('test1', 'eval_models/weights/v0.1/mtviton.pth', 0, 'eval_models/weights/v0.1/gen.pth', 'unpaired', 't2.txt', './test')
    # main()